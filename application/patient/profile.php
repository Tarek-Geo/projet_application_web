            <?php 
                $bdd = new PDO('mysql:host=localhost;dbname=medical','root','', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
                 if (isset($_POST['nom']))
                {
                    $requete = $bdd->prepare('UPDATE patient SET NOM_P=? where PATIENT_ID=?');
                    $requete->execute(array($_POST['nom'],$_SESSION['user_session']));
                   
                }
                if (isset($_POST['email']))
                {
                    $requete = $bdd->prepare('UPDATE patient SET EMAIL_P=? where PATIENT_ID=?');
                    $requete->execute(array($_POST['email'],$_SESSION['user_session']));
                   
                }
                $requete_f = $bdd->prepare('SELECT NOM_P,EMAIL_P FROM patient where PATIENT_ID=?');
                    $requete_f->execute(array($_SESSION['user_session']));
                while ($donnees_f = $requete_f->fetch()){
                    $nom=$donnees_f['NOM_P'];
                    $email=$donnees_f['EMAIL_P'];
                }
            ?>
           ...
           <?php
    {
        if(isset($_POST['password'],$_POST['new_password'],$_POST['confirm_new_password']))
        {
            ...
            if ($_POST['new_password']==$_POST['confirm_new_password']) {
                $requete_s = $bdd->prepare('SELECT PASSWORD_P FROM patient where PATIENT_ID=?');
                $requete_s->execute(array($_SESSION['user_session']));
                $donnees_s = $requete_s->fetch();
                    if($donnees_s['PASSWORD_P']==$pass) {
                        $requete = $bdd->prepare('UPDATE patient SET PASSWORD_P =? where PATIENT_ID=?');

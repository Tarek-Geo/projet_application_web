...
">Infirmière</label>
                         <div class="col-sm-5">
                            <select name="inf" class="form-control" placeholder="Selectionner infirmière" style="
    width: 218px;
">                          
                                <?php
...
">Description</label>
    ...
    {
        $nomm="";
        $requete_s = $bdd->prepare('SELECT NOM_M FROM medecin WHERE MEDECIN_ID=?');
        $requete_s->execute(array($user_id));
        while($donnees = $requete_s->fetch()){$nomm=$donnees['NOM_M'];}
        ...
        if (isset($_POST['patient'],$_POST['inf'],$_POST['c_date'],$_POST['description']))
        {
            $requete = $bdd->prepare('INSERT INTO consultation(
                                    ...
                                    ');
            $requete->execute(array($_POST['patient'],$nomm,$_POST['inf'],$_POST['c_date'],$_POST['description']));
        }
    }
    ?>
    </div>
